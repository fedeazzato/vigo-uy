...
- **Curated content** — hand-maintained by the site owner, lives in JSON files under `src/data/`, imported directly into page components. Adding or editing this content never requires touching React code. It is being progressively retired in favor of community data via the `preferCommunity()` gate — see `specs/CONTENT-MIGRATION.md` for the per-file plan and thresholds.
...
- **Community content is publicly readable (anon role), submitting requires auth.** The deliberate anon exposure surface is exactly: public non-hidden content rows from non-banned authors, `public_profiles` (id + display_name only), the two stats views, `vehicle_km_leaderboard`, `community_totals`, and `is_user_banned(uuid)` (anon-executable **by design** — the anon RLS policies call it and policy expressions run with the caller's privileges, so EXECUTE can't be revoked; UUIDs aren't enumerable, accepted risk, do not re-flag). `vehicles.join_code` must never appear in any anon-reachable relation. `profiles` itself is own-row readable only (since migration 0021) — cross-user display names must come from `public_profiles` or, for moderation (incl. banned users), `admin_list_users()`. Shared fetch helpers live in `src/lib/communityData.ts` — use them instead of re-querying from pages, and note author names must come from `public_profiles` (not `profiles`) so they resolve for signed-out visitors.
