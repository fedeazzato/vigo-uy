-- Reported gap in site search (0028, specs/site-search.md): users often put
-- their odometer reading as the service_entries service_type (e.g. "45.000
-- km") instead of a descriptive word, so searching "service" matched the
-- Mantenimiento guide page but none of the actual submitted service
-- records -- nothing in that row's own fields ever contained the word.
--
-- Fix: append a couple of fixed Spanish keywords per entry kind to each
-- arm's indexed tsvector, on top of the row's own fields, so a generic
-- "what kind of thing is this" search always finds it regardless of what
-- the author actually typed. These keywords feed only the match
-- expression -- title/subtitle (what the UI shows) are untouched.
...
as $$
  ...
  from (
    select
      ...
      s.created_at,
      ts_rank(
        to_tsvector('spanish', 'service taller ' || coalesce(s.service_type, '') || ' ' || coalesce(s.dealer, '') || ' ' || coalesce(s.city, '') || ' ' || coalesce(s.notes, '')),
        websearch_to_tsquery('spanish', coalesce(search_query, ''))
      ) as rank
    from public.service_entries s
    where s.is_public and not s.hidden
      and to_tsvector('spanish', 'service taller ' || coalesce(s.service_type, '') || ' ' || coalesce(s.dealer, '') || ' ' || coalesce(s.city, '') || ' ' || coalesce(s.notes, ''))
        @@ websearch_to_tsquery('spanish', coalesce(search_query, ''))
